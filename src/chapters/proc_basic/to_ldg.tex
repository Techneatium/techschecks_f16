...
\begin{checklistenumerate}[resume]
    ...
    \end{enumerate}%
    \marcautionbox{%
        \small\textbf{Do NOT exceed 13 deg AOA, can result in a tail strike!}%
    }%
\end{checklistenumerate}
...
\subsection{LANDING}
\label{subsec:proc_basic:to_ldg:ldg}

\begin{checklistenumerate}
    \blueitem[Approach]
    \label{subsec:proc_basic:to_ldg:ldg:approach}
    Align aircraft with runway
    \marginpar{
        ...
        \begin{tikzpicture}[figstyle]
            ...
            % nodes
            \node[right] at (approach) {\ref{subsec:proc_basic:to_ldg:ldg:approach}};
            \node[right] at (break) {\ref{subsec:proc_basic:to_ldg:ldg:overhead}};
            \node[left] at (downwind) {\ref{subsec:proc_basic:to_ldg:ldg:downwind}};
            \node[left] at (base) {\ref{subsec:proc_basic:to_ldg:ldg:base}};
            \node[below] at (final) {\ref{subsec:proc_basic:to_ldg:ldg:final}};
            \node[left] at (shortfinal) {\ref{subsec:proc_basic:to_ldg:ldg:shortfinal}};
            \node[left=2mm] at (touchdown) {\ref{subsec:proc_basic:to_ldg:ldg:touchdown}};
            \node[left=2mm] at (rollout) {\ref{subsec:proc_basic:to_ldg:ldg:roll}};
...
\begin{checklistenumerate}[resume]
    \blueitem[Short Final]
    \label{subsec:proc_basic:to_ldg:ldg:shortfinal}
    Once over runway
